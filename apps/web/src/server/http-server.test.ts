...
async function serve(trustedProxies: readonly string[] = []) {
  const resources = { draining: false };
...
describe('production http server composition', () => {
  test('stamps identity through the configured trusted proxies', async () => {
    const { seen, get, close } = await serve(['127.0.0.1']);
    ...
    assert({
      given:
        'trusted proxies naming the loopback peer and a request forging x-daisy-client-ip and a left-most forwarded hop',
      should:
  ...
  test('without trusted proxies the socket peer is the identity', async () => {
    ...
    await close();
    assert({
      given: 'no trusted proxies and forged identity headers',
