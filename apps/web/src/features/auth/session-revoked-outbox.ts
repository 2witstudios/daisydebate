...
const REVOKE_PATHS = new Set(['/revoke-session', '/revoke-other-sessions']);
 ...
 * not one per session revoked: `/revoke-other-sessions` may delete several
 * sessions at once and Better Auth's response does not say how many, and
 * public/owner-topic payloads are doorbells, never enumerated deltas.
    ...
    after: [
      {
        ...
        handler: createAuthMiddleware(async (context) => {
          ...
          if (succeeded && typeof userId === 'string')
            await appendSessionRevoked(userId);
        }),
      },
    ],
  },
