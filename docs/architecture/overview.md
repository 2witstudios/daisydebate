...
| `packages/db`                | Drizzle schema, migrations, transactional record adapters; CHECK vocabularies derive from protocol enums (ADR 0029)                                                                                                                                                                                                                                                                                              | config, errors, protocol                                                                                                   | platform/data     |
| `packages/redis`             | Namespaced ephemeral key operations and lifecycle                                                                                                                                                                                                                                                                                                                                                                | config, errors, protocol                                                                                                   | platform/data     |
| `packages/auth`              | Principal/permission vocabulary (`debate:create`, `debate:read`, `debate:manage`; each stands alone, none implies another), the username rule, and `resolveIdentity` (verified session facts → anonymous / provisional / member Principal; members hold `debate:create` only); trusted authentication adapters plug in through an injected session reader; never imports Better Auth or any framework (ADR 0017) | errors                                                                                                                     | identity owner    |
