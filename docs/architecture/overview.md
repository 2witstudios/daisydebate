...
| `packages/redis`             | Namespaced ephemeral key operations and lifecycle                                                                                                                                                                                                                                                                                                                                                                | config, errors                                                                                                             | @2witstudios |
...
`presence` in the `apps/realtime` row is not a package yet: ADR 0031 §12
pre-declares the edge in `scripts/boundaries-rules.ts` so nothing else moves
when a later realtime leaf adds `packages/presence`.
