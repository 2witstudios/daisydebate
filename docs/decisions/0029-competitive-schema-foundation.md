...
2. **Account deletion tombstones the user.** `users.deleted_at` plus the
   ...
   transaction. No delete endpoint ships with this ADR.
...
4. **Projections are written in the same transaction as their source.**
   ...
   separately), stamps `started_at` on the first save that leaves
   `waiting`, and `completed_at` plus the caller's `outcome` on the save
   that completes; a completing save without an outcome is refused before
   any statement runs. `debate_participants` is the same kind of
...
8. **Glicko-2, one debate is one rating period.** `ratings` holds
   ...
   rewrites history. A `rating_period_id` would be an additive column later.
   `seasons` allows at most one `active` row (partial unique index).
...
12. **`@daisy/db` may depend on `@daisy/protocol`.** The only new package
    ...
    projection all come from protocol.
...
- `formats` is reference data seeded by `scripts/seed.ts`
  (`seed_versions` marker `formats`); the `foundation` format is
  deliberately unjudged (`judge: 0`) because the engine has no judge
  operation. `scripts/agent-seed.ts` seeds one actor per agent user and the
  seed debate's author is an actor (`agent-seed-v3`).
- `packages/db/integration/competitive-constraints.integration.ts` proves a
  rejecting and an accepting case for every CHECK, UNIQUE and FK named here.
