...
- Worktree agents never write the task board directly. The session that
  ...
  Artifact pages are different: a worktree agent publishes its own handoff
  or review record in the drive's `Plans`/`Reviews` folders, because those
  are the record a reviewer reads. `/tmp` and agent-local notes are scratch.
