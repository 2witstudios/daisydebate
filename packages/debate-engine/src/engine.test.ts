...
describe('legacy identity compatibility', () => {
  ...
  const legacySnapshot = {
    ...
    participants: [
      { id: legacyUserId, side: 'affirmative' as const, ready: false },
    ],
  };

  test('restores durable pre-cuid2 snapshots without reminting identity', () => {
    const restored = restoreDebateRuntime(
      JSON.parse(JSON.stringify(legacySnapshot)),
    );
    assert({
      given: 'a legacy UUID snapshot restored from durable storage',
      should: 'preserve every identity verbatim',
      actual: restored.snapshot(),
  ...
  test('still rejects identifiers outside the documented shapes', () => {
    expect(() =>
      restoreDebateRuntime({
        ...legacySnapshot,
        id: legacyDebateId.toUpperCase(),
      }),
    ).toThrow();
    expect(() =>
      restoreDebateRuntime({ ...legacySnapshot, id: 'short' }),
    ).toThrow();
  });
});
